...
if [[ -z $USERNAME_RESULT ]]; then
    ...
    # get games played and best game (fewest guesses)
    GAMES_PLAYED=$($PSQL "SELECT COUNT(*) FROM games JOIN users USING(user_id) WHERE username='$USERNAME'")
    BEST_GAME=$($PSQL "SELECT MIN(guesses) FROM games JOIN users USING(user_id) WHERE username='$USERNAME'")

    # print welcome back message with games played and best game
...
until [[ $USER_GUESS -eq $SECRET_NUMBER ]]; do
    ...
    if [[ ! $USER_GUESS =~ ^[0-9]+$ ]]; then
        ...
        # check if guess is higher or lower and give hint
